  ...
  return (
    <Card {...props} className="p-3 hover:outline hover:outline-1  shadow-md  hover:outline-blue-600">
      <div className="flex gap-3 items-start">
        ...
        <div className="flex flex-col ">
          <Link href={`/paket/${data?.slug}`} key={index} title={data?.title}>
            <span className="text-sm font-semibold line-clamp-1 hover:text-blue-600">{data?.title}</span>
          </Link>
          <div className="flex justify-between">
            <span className="text-sm text-orange-400 font-bold">{formatRupiah(data?.price)}</span>
      ...
      <div className="my-2">
        ...
        <Progress className="" value={data?.quota - Math.floor(data?.quota / 2)} max={data?.quota} />
      </div>
      <Separator />
      <div className="flex justify-between my-2">
        ...
        <span className="text-sm flex gap-2 items-center">
          {data?.hotel_class} <FaRegStar className="text-yellow-500" /> <FaHotel />
        </span>
      </div>
      <div className="flex justify-between my-2">
        <span className="text-sm flex gap-2 items-center">
      ...
      <div className="flex justify-between items-center my-3">
        ...
        <div className="flex items-center gap-1">
          <OrderButton />
          <CompareButton id={data?.id ?? ''} logged={data?.is_logged} slug={data?.slug} compared={data?.is_compared} title={data?.title} />
          <Favorites data={data} />

          <ShareButton image_url={data?.gallery?.image_url} title={data?.title} url={'/paket/' + data?.slug} />
        </div>
      </div>
    </Card>
  );
