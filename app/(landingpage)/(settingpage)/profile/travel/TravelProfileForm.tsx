...
export default function TravelProfileForm({ travelProfile, countries, provinces, cities }: { travelProfile: any; countries: any; provinces: any; cities: any }) {
  ...
  const handleImage = (e: any) => {
    if (!e.target.files[0]) return setUrlImage(null);
    ...
    <form action="" method="post" className="flex gap-3 md:divide-x w-auto max-md:flex-col max-md:flex-col-reverse">
      <div className="grid gap-3 w-full">
        <div className="grid items-center gap-1.5">
          ...
          <Input id="name" type={'text'} placeholder="Masukkan Email anda..." defaultValue={travelProfile.name} />
        </div>
        <div className="grid items-center gap-1.5">
          <Label htmlFor="description">Deskripsi</Label>
          <Textarea id="description" className="resize-none" placeholder="Deskripsikan tentang travel anda..." rows={4} defaultValue={travelProfile.description} />
        </div>
        <div className="grid items-center gap-1.5">
          <Label htmlFor="email">Email</Label>
          <Input id="email" type={'email'} placeholder="Masukkan Email anda..." defaultValue={travelProfile.email} />
        </div>
        <div className="grid items-center gap-1.5">
        ...
        <div className="grid items-center gap-1.5">
          ...
          <Input id="postal_code" type={'number'} placeholder="Masukkan Kode Pos..." defaultValue={travelProfile.postal_code} />
        </div>
        <div className="grid items-center gap-1.5">
          <Label htmlFor="address">Alamat</Label>
          <Textarea id="address" className="resize-none" placeholder="Masukkan deskripsi disini..." rows={4} defaultValue={travelProfile.address} />
        </div>
        <div>
