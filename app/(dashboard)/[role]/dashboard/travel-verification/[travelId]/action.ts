...
export async function GetTravel(id: string) {
  ...
  if (!res.ok || res.status !== 200) return redirect(`/${session?.user.role}/dashboard/travel-verification`);
...
export async function downloadCredentialTravel(url: string, title: string) {
  return process.env.IMAGE_DOWNLOAD_API + '/api/download/image?url=' + url;
}
