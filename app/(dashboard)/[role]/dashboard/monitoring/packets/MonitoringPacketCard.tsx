...
export default function MonitoringPacketCard({ data }: { data?: any }) {
  return (
    <Card className="">
      ...
      <CardHeader>
        <CardTitle>
          <Link title={data?.title} href={'packets/' + data?.id} className="hover:text-blue-600 line-clamp-1">
            {data?.title}
          </Link>
        </CardTitle>
      </CardHeader>
        ...
        <div className="outline-dashed outline-offset-5 outline-1 outline-gray rounded-lg p-5 bg-gray-100 flex justify-between">
          <div className="flex items-center relative min-h-[40px] w-full">
            {data?.comparison &&
              data?.comparison.map((user: any, index: number) => (
                <Image
                  ...
                  width={100}
                  className={cn('object-cover outline cursor-pointer outline-1 outline-blue-dark bg-fixed w-40 h-40 rounded-full', 'z-' + (index + 1) * 10, '-mr-[1.5rem]')}
                  style={{
                    objectFit: 'cover',
                    width: '40px',
                    height: '40px',
            ...
            {data?.comparison_count > data?.comparison?.length && (
              <div className={cn('object-cover relative outline bg-yellow-400 cursor-pointer outline-1 outline-blue-dark w-10 h-10 rounded-full z-40', '-mr-[1.5rem]')}>
                <div className="absolute inset-0">
                  <span className="text-black/70 items-center flex w-full h-full justify-center" title={data?.comparison_count - data?.comparison?.length + ' Lainnya'}>
                    +{data?.comparison_count - data?.comparison?.length}
                  </span>
                </div>
              </div>
            )}
            {data?.comparison_count === 0 && <span className="flex items-center justify-center">Belum ada yang membandingkan.</span>}
          </div>
