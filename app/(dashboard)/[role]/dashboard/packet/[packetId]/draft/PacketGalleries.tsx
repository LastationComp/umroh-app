...
export default function PacketGalleries({
  images,
  packetId,
}: {
  images: any[];
  ...
  const handleUploadImage = () => {
    startTransition(async () => {
      ...
      if (fileImage.current?.files)
        formData.set("image", fileImage.current?.files[0]);
      ...
      >
        <CarouselContent className="w-auto">
          {images.map((image, index) => (
            <CarouselItem
              key={index}
              className="basis-12/12 md:basis-6/12 lg:basis-3/12 xl:basis-2/12 max-w-[200px]"
            >
              <div className="p-3 cursor-pointer bg-white shadow border border-1 flex items-center trigger-tooltip-image relative ">
                ...
                <section className="absolute top-4 right-4 tooltip-image">
                  <DeleteButton
                    ...
                    refreshOnTag={"travel-packet-galleries"}
