...
export default function VariantsForm({ data = [] }: { data?: any[] }) {
  const [variants, dispatch] = useReducer(
    arrayReducer,
    data.length !== 0 ? data : initialData
  );
  ...
  const addOns = () => {
    const find = variants?.find(
      (variant: any) => variant.slug === Slug(packetName)
    );
              ...
              <div className="flex items-center gap-3 justify-between">
                ...
                {index !== 0 && (
                  <Button
                    variant={"destructive"}
                    type="button"
                    onClick={() => removeVariant(varian?.title)}
                  >
                    Hapus
                  </Button>
                )}
              ...
              <div className="flex items-center gap-3 justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex items-center space-x-2">
                    <Switch
                      id={activeVariantDetailsId + index}
                      defaultChecked={varian?.is_active ?? true}
                      name={inputName("variants", index, "is_active")}
                    />
                    <Label htmlFor={activeVariantDetailsId + index}>
                      Aktif
                    </Label>
                  </div>
                </div>
                <input
                  type="hidden"
                  name={inputName("variants", index, "id")}
                  value={varian?.id ?? index}
                  readOnly
                />
                <input
                  type="hidden"
                  name={inputName("variants", index, "title")}
                  value={varian?.title ?? ""}
                  readOnly
                />
                <input
                  type="hidden"
                  name={inputName("variants", index, "is_primary")}
                  value={varian?.is_primary ?? false}
                  readOnly
                />
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    id="multi-choice"
                    disabled={index === 0}
                    defaultChecked={varian?.type === "multiple"}
                    name={inputName("variants", index, "type")}
                  />
              ...
              <div className="flex flex-col gap-3">
                  ...
                  <div className="grid gap-1" key={indexDetail}>
                    <div className="flex gap-3 items-center" key={indexDetail}>
                      <Input
                        ...
                        placeholder="Masukkan Opsi Varian Paket disini..."
                      />
                      ...
                      <input
                        ...
                        readOnly
                        value={detail?.id ?? indexDetail}
                      />
                    </div>

                    <div className="flex items-center space-x-2">
                      <Switch
                        id={activeVariantDetailsId}
                        defaultChecked={detail?.is_active ?? true}
                        name={inputName(
                          "variants_details",
                          index,
                          indexDetail,
                ...
                {index !== 0 && (
                  <div className="flex gap-3 items-center">
                    <Button
                      onClick={() => addOptionAddons(varian?.title ?? "")}
                      type={"button"}
                    >
                      Tambah
                    </Button>
                    {varian?.details?.length > 1 && (
                      <Button
                        type="button"
                        onClick={() => removeOptionAddons(varian?.title ?? "")}
                        variant={"destructive"}
                      >
                        Hapus
                      </Button>
