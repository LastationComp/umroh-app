...
const DraftCard = dynamic(() => import("./DraftCard"));
const PublishPacket = dynamic(() => import("./PublishPacket"));
...
export default function Packets({ travel }: { travel: any }) {
  ...
  const { data: drafts } = useSWR(
    ["/api/dashboard/travel/packets?is_publish=0", draftPage],
    ([url, paginate]) => fetcher(url + "&paginate=12&page=" + paginate)
  );
  const { data: publish } = useSWR(
    ["/api/dashboard/travel/packets?is_publish=1", publishPage],
    ([url, paginate]) => fetcher(url + "&paginate=12&page=" + paginate)
  );
    ...
    <section className="flex flex-col gap-3">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {drafts?.data?.length === 0 && (
          <span className="lg:col-span-4 text-center">
            Tidak ada Paket Draft.
          </span>
        )}
        {drafts &&
        ...
        <section className="col-span-full flex justify-between gap-3 w-full">
          ...
          <div className="flex gap-3">
            <Button
              onClick={() => decPage("draft")}
              disabled={!drafts?.prev_page_url}
            >
              <ChevronLeftIcon className="h-4 w-4" />
            </Button>
            <Button
              onClick={() => incPage("draft")}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        ...
        {publish?.data?.length === 0 && (
          <span className="lg:col-span-4 text-center">
            Tidak ada Paket yang di publish.
          </span>
        )}
        {publish?.data &&
        ...
        <section className="col-span-full flex justify-between gap-3 w-full">
          ...
          <div className="flex gap-3">
            <Button
              onClick={() => decPage("publish")}
              disabled={!publish?.prev_page_url}
            >
              <ChevronLeftIcon className="h-4 w-4" />
            </Button>
            <Button
              onClick={() => incPage("publish")}
              disabled={!publish?.next_page_url}
            >
              <ChevronRightIcon className="h-4 w-4" />
            </Button>
          </div>
