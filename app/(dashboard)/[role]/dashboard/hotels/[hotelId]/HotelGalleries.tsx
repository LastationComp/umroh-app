...
export default function HotelGalleries({ images, hotelId }: { images: any[]; hotelId: string }) {
  ...
  const handleUploadImage = async (formData: FormData) => {
    const result = await uploadGallery(hotelId, formData);
    await delay(1000);
  ...
  return (
    <section className="w-auto">
      <span className="font-bold">Gambar Hotel</span>
            ...
            <CarouselItem key={index} className="basis-12/12 md:basis-6/12 lg:basis-3/12 xl:basis-2/12 ">
              <div className="p-3 cursor-pointer bg-white shadow border border-1 flex items-center trigger-tooltip-image relative ">
                ...
                <section className="absolute top-4 right-4 tooltip-image">
                  <DeleteButton endpoint={'/api/dashboard/hotels/' + image.hotel_id + '/galleries/' + image.id + '?url=' + image.image} message="Apakah anda ingin menghapus gambar ini?" />
                </section>
              </div>
            </CarouselItem>
          ))}
